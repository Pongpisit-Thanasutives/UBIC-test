...
from weakident_utils.calculations import find_corner_pts
...
def build_feature_vector_tags(
    ...
    # c_true = compute_true_coefficient_vector(n, true_coefficients, lhs_tags, dict_list, lhs_ind)
    rhs_ind = np.array(
        [i for i in range(dict_list.shape[0]) if i not in lhs_ind])
    return dict_list, lhs_ind, rhs_ind
