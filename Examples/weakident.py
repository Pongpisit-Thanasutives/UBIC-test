...
from weakident_utils.helpers import set_hist_bins, set_sparsity_level
import numpy as np
import scipy.linalg
...
def weak_ident(
        ...
        tau, use_cross_der: bool = False) -> Tuple[np.array, np.array, pd.core.frame.DataFrame, float]:
    # Expanding dims
    if len(u_hat.shape) == 2:
        u_hat = np.expand_dims(u_hat, axis=0)
    ...
    dictionary_list, idx_of_lhs_feature, idx_of_rhs_feature  = build_feature_vector_tags(
        num_of_variables, dim_x, max_dx, max_poly, use_cross_der, is_1d_ode)
    ...
    latex_tags_lhs, latex_tags_rhs = build_feature_latex_tags(num_of_variables, dim_x, is_1d_ode, dictionary_list, idx_of_lhs_feature)

    return w, b, c_pred, latex_tags_lhs, latex_tags_rhs
